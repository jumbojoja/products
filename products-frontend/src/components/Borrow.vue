<template>
    <el-scrollbar height="100%" style="width: 100%;">
        ...
        <el-table v-if="isShow" :data="fitlerTableData" height="600"
            ...
            <el-table-column prop="borrowTime" label="借出时间" :formatter="formatDate" sortable />
            <el-table-column prop="returnTime" label="归还时间" :formatter="formatDateReturn" sortable />
        </el-table>
...
export default {
    ...
    methods: {
        async QueryBorrows() {
            this.tableData = [] // 清空列表
            let response = await axios.get('/borrowHistory', { params: { cardID: this.toQuery } }) // 向/borrow发出GET请求，参数为cardID=this.toQuery
        ...
        formatDate(row, column) {
            ...
            let date = new Date(parseInt(row.borrowTime));
        ...
        formatDateReturn(row, column) {
            if (row.returnTime  == "0") {
                return "----------";
            }
            let date = new Date(parseInt(row.returnTime));
            let Y = date.getFullYear() + '.';
            let M = date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) + '.' : date.getMonth() + 1 + '.';
            let D = date.getDate() < 10 ? '0' + date.getDate() + ' ' : date.getDate() + ' ';
            let h = date.getHours() < 10 ? '0' + date.getHours() + ':' : date.getHours() + ':';
